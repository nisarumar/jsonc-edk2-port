...
#define printbuf_memappend_checked(p, s, l)                   \
	do {                                                  \
		if (printbuf_memappend((p), (s), (int)(l)) < 0)    \
		{                                             \
	...
	redo_char:
		...
		case json_tokener_state_escape_unicode:
		{
			...
			else if (tok->ucs_char < 0x800)
			{
				...
				unescaped_utf[0] = (unsigned char) (0xc0 | (tok->ucs_char >> 6));
				unescaped_utf[1] = (unsigned char) (0x80 | (tok->ucs_char & 0x3f));
			...
			else if (tok->ucs_char < 0x10000)
			{
				...
				unescaped_utf[0] = (unsigned char) (0xe0 | (tok->ucs_char >> 12));
